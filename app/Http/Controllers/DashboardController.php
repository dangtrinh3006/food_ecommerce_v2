...
class DashboardController extends Controller
{
    ...
    public function changepassw(Request $req)
    {
        ...
        if (Hash::check($oldpass, $getPass)) {
            if ($newpass !== $confirm) {
                return redirect()->back()->with('failStatus', 'Nhập lại mật khẩu không khớp.');
            } else {
                ...
                return redirect()->back()->with('successStatus', 'Cập nhật mật khẩu thành công.');
            }
        } 
        return redirect()->back()->with('failStatus', 'Mật khẩu cũ không đúng.');
